<template>
  <div>
    <h1 class="text-3xl font-bold text-[#1A2036] anim-field" style="animation-delay:0.05s">Welcome back</h1>
    ...
    <form class="mt-8 space-y-4" @submit.prevent="onSubmit">
      <div class="flex items-center gap-3 rounded-full bg-[#F3F4F6] px-5 py-3.5 ring-2 ring-transparent transition-all duration-200 focus-within:ring-[#3D5FE0]/40 focus-within:bg-white anim-field" style="animation-delay:0.15s">
        ...
        <input v-model="form.email" type="email" required placeholder="Email address" class="w-full bg-transparent text-sm text-[#1A2036] placeholder:text-[#9CA3AF] outline-none" />
      </div>

      <div class="flex items-center gap-3 rounded-full bg-[#F3F4F6] px-5 py-3.5 ring-2 ring-transparent transition-all duration-200 focus-within:ring-[#3D5FE0]/40 focus-within:bg-white anim-field" style="animation-delay:0.2s">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" class="h-5 w-5 shrink-0 text-[#9CA3AF]">
        ...
        <input v-model="form.password" :type="showPassword ? 'text' : 'password'" required placeholder="Password" class="w-full bg-transparent text-sm text-[#1A2036] placeholder:text-[#9CA3AF] outline-none" />
        <button type="button" class="shrink-0 text-[#9CA3AF] transition-colors duration-150 hover:text-[#3D5FE0]" @click="showPassword = !showPassword">
      ...
      <div class="pt-1 text-right anim-field" style="animation-delay:0.25s">
        <RouterLink to="/forgot-password" class="text-sm font-medium text-[#3D5FE0] transition-colors hover:text-[#3350C0]">Forgot password?</RouterLink>
      </div>
    ...
    <p class="mt-8 text-center text-sm text-[#6B7280] anim-field" style="animation-delay:0.45s">
      Don't have an account?
      <RouterLink to="/register" class="font-semibold text-[#3D5FE0] transition-colors hover:text-[#3350C0]">Register</RouterLink>
    </p>
  </div>
</template>
...
const onSubmit = async () => {
  ...
  try {
    ...
    errorMessage.value = err.response?.data?.message || err.response?.data?.error || 'Login failed. Please check your email and password.'
  } finally {
    loading.value = false
  }
}
